...
import { addLickageData } from "../../store/apiCall/tokenize";
...
export default function AddLickageData({ projectDetails, handleNext, projectId, handleBack }) {
    ...
    const formik = useFormik({
        ...
        onSubmit: (values) => {
            ...
            dispatch(addLickageData(projectId, setLoading, payload, () => {
                handleNext(true);
            }))
        }
    });
    ...
    if (projectDetails?.audit?.leakage?.length > 0) {
        return (
            <Grid container spacing={2}>
                <Grid size={{ xs: 12 }}>
                    <Typography variant="h6">All modules Leakage Reports are Submitted.</Typography>

                </Grid>
                {projectDetails?.audit?.leakage
                    .map((m, index) => (
        ...
        return (
            <Paper sx={{ p: 3, mb: 3 }}>
                <Typography variant="h6" gutterBottom>
                    The user is yet to Add Leakage data to the project.
                </Typography>
                                        ...
                                        <Paper sx={{ p: 2 }}>
                                            ...
                                            <FormGroup>
                                                <FormControlLabel
                                                    control={
                                                        <Checkbox
                                                            name={`emissionData.${index}.finalize`}
                                                            checked={module.finalize}
                                            ...
                                            {moduleTouched.finalize && moduleErrors.finalize && (
                                                <Typography color="error" variant="caption">
                                                    {moduleErrors.finalize}
                                                </Typography>
                                            )}
